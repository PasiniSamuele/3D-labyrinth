...
var S3 = `
	...
	float CosAlpha = dot(OlightDir, Dir);
	float CosOut = cos(radians(ConeOut)/2.0);
	float CosIn = cos(radians(ConeOut)*radians(ConeIn)/2.0);

	OlightColor = lightColor * clamp((CosAlpha - CosOut) / (CosIn - CosOut), 0.0, 1.0);
...
var S5 = `
	...
	OlightColor = lightColor * pow(Target/length(Pos - fs_pos) , Decay) * clamp((CosAlpha - CosOut) / (CosIn - CosOut), 0.0, 1.0);
`;
...
var S6 = `
	...
	vec4 upComponent = ((dot(normalVec, ADir) + 1.0)/2.0)*ambientLightColor;
	vec4 downComponent = ((-dot(normalVec, ADir) + 1.0)/2.0)*ambientLightLowColor;

	ambientColor = upComponent + downComponent;
`;
